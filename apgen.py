...
# V.A. Moss 06/11/2017 (vmoss.astro@gmail.com)
...
# Default file to use:
if obstype == 'single':
	usefile = 'create_parset.txt'
elif obstype == 'multi':
	usefile = 'create_parset_multi_element_beams.txt'
else:
	print 'Error! Neither of these obstypes exist...'
	sys.exit()

# Deal with the software version
if obstype == 'single' and softver != '2.6':
	f = open('create_parset.txt','rU').read()
	f2 = ('SR="%s"' % softver).join(f.split('SR="2.6"'))
	out = open('temp_create.txt','w')
	out.write(f2)
	out.flush()
	usefile = 'temp_create.txt'
elif obstype == 'multi' and softver != '2.6':
	f = open('create_parset_multi_element_beams.txt','rU').read()
	f2 = ('SR="%s"' % softver).join(f.split('SR="2.6"'))
	out = open('temp_create.txt','w')
	out.write(f2)
	out.flush()
...
if obstype == 'multi':
	...
	outname = '%s_params.txt' % fname.split('.')[0]
	outname2 = '%s_params.sh' % fname.split('.')[0]
	os.system('cat %s %s.txt > %s' % (outname,usefile,outname2))
